...
#define ZTEMT_TP_WAKEUP_GESTURE_FUNCTION	1	
...
static inline size_t cyttsp4_bits_2_bytes(int nbits, size_t *max)
{
	*max = 1 << nbits;
	return (nbits + 7) / 8;
}
...
static CLASS_ATTR(ts_info, S_IRUSR, class_cyttsp4_ts_info_show, NULL);
...
static int cyttsp4_core_probe(struct cyttsp4_core *core)
{
	...
	if (rc < 0) {
		dev_err(dev, "%s class_create_file failed!\n", __func__);
	...
	ts_info = devm_kzalloc(dev, CY_INFO_MAX_LEN, GFP_KERNEL);
	if (!ts_info) {
		dev_err(dev, "Not enough memory\n");
		goto err_debug_dir;
	}
